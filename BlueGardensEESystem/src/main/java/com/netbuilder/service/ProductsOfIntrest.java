...
/**
 * used to create and hold the 'hot' products. This is the products of interest
 * class inside the service layer. This will contain all the logic behind
 * selecting which products will be displayed on specific pages.
 * 
...
public class ProductsOfIntrest {
	...
	/**
	 * creates 3 lists of three unique products for products on sale, pourous
	 * and trending that can be accessed through getters : getSale(),
	 * getPourous() and getTrending().
	 */
	...
	/**
	 * This method determines which determines which of the 3 lists are the
	 * smallest and returns them in ascending order. index 0 = the smallest
	 * array index 1 = the second smallest array index 2 = the largest array (if
	 * they are all the same length then the order will be saleList, newList,
	 * trendingList)
	 * 
	...
	/**
	 * @author Jake Check which fields are entered by the user and return the
	 *         one with the most results first In stock = most results name =
	 *         fewer results id = only one result
	 */
	public ArrayList<Product> getSearchResults(int id, String name,
			boolean stock) {
