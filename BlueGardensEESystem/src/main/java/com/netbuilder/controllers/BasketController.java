...
public class BasketController implements Serializable {
	...
	public ArrayList<Product> getItemsFromPOLS() {
		...
		for (int i = 0; i < bm.products().size(); i++) {
			temp.add(productManager.findById(bm.products().get(i)
					.getProductId()));
		}
		return temp;
	}
...
/*
 * dispay basket on page from contoller (thep page doesnt know what to do with a
 * basket) you need to go to the se4rvice layer from the tte service you need to
 * got o the basket managfer, get the basket, bull out the products. send back
 * to controller as array XHTML have some dynamic table thing to display each
 * element in array.
 */