...
/**
 * 
 * @author gandrews Checks that product methods are correctly tested
 */
public class ProductTest {
	...
	/**
	 * Added tests for whether a product is in sale along with the features and
	 * trending
	 * 
	 * @author lczornyj
	 */
	 ...
	 */
	// Desired Values Name Stock Level Minimum Level Recommended Level
	// Porousware Cost
	Product product = new Product(testName, testStockLevel, testMinimumLevel,
			testRecommendedLevel, testPorouswareApplied, testCost, testSale,
			testFeatureOne, testFeatureTwo, testFeatureThree, testFeatureFour,
			testFeatureFive, testImageLink, testTrending);
	Product productError = new Product(testNameError, testStockLevelError,
			testMinimumLevelError, testRecommendedLevelError,
			testPorouswareAppliedError, testCostError, testSaleError,
			testFeatureOneError, testFeatureTwoError, testFeatureThree,
			testFeatureFourError, testFeatureFiveError, testImageLinkError,
			testTrendingError);
	...
	public void testProductCustomCreation() {
		...
		assertEquals(product.getPrice(), 10.00, 0.001); // 0.001 is the
														// tolerance
	...
	/**
	 * Checks to see that the product minimum threshold has been assigned and is
	 * not left null
	 */
	@Test
	...
	/**
	 * Checks that the product is correctly assigned if it is coated in
	 * porousware or not
	 */
	@Test
	...
	/**
	 * Test that the products name does not exceed the maximum number of
	 * characters allowed
	 */
	...
	public void testProductCostNotEqual() {
		assertNotEquals(productError.getPrice(), 17.00, 0.001); // 0.001 is the
																// tolerance
	}

	/**
	 * Test that the products name does not accept values that exceed the
